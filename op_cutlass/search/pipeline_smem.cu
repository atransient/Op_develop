...
template <class ProblemShape, class CtaTiler, class TA, class SmemLayoutA, class TmaA, uint32_t NumStages>
__global__ void test0(ProblemShape gmem_shape, CtaTiler cta_tiler, TA const *A, CUTLASS_GRID_CONSTANT TmaA const tma_a)
{
    //gmem tensor init
    ...
    if (lane_predicate && (warp_idx == 0))
    {
        ...
        for ( int i = 0; i < prologue_iterations; ++i)
        {
            ...
            copy(tma_a.with(*tma_barrier), tAgA(_,i), tAsA(_,i));
    ...
    for (int i = 0; i < num_iterations; ++i)
    {
        ...
        if (tile_count != 0 && lane_predicate && warp_idx == 0)
        {
            ...
            BarrierType* tma_barrier = pipeline.producer_get_barrier(smem_pipe_write);
            // Can also specify stage to commit directly
            copy(tma_a.with(*tma_barrier), tAgA(_, write_index), tAsA(_, write_index));
            // pipeline.producer_commit(smem_pipe_write);
            --tile_count;
            ++smem_pipe_write;
        }
...
template <class TA>
void intermedia_config(int M, int N, TA const* A)
{
    ...
    dim3 grid(1,1,1);
    test0<decltype(gmem_shape), decltype(cta_tiler), TA, decltype(sA), decltype(tmaA), num_stage><<<grid, block, smem_size>>>(gmem_shape, cta_tiler, A, tmaA);
    CUTE_CHECK_LAST();
}
...
int main() {
    ...
    intermedia_config<TA>(m,n,d_A.data().get());
    CUTE_CHECK_LAST();
}

...
void bpw_be_devicec(cudnnHandle_t &handle, vector<uint>v_info, uint64_t X_ptr, uint64_t dY_ptr, uint64_t dW_ptr, cudnnDataType_t iodtype, cudnnDataType_t cdtype) {
    ...
    cudnnTensorDescriptor_t input_desc, grad_desc;
    cudnnFilterDescriptor_t filter_desc;
    cudnnConvolutionDescriptor_t conv_desc;

    
    auto [oup_size, workspace_size, algo] = be_bpw_desc(handle, input_desc, grad_desc, filter_desc, conv_desc, {n,c,h,w,k,r,s,stride,dilation,padding,groups}, iodtype, cdtype);
   
    void *d_X = reinterpret_cast<void *>(X_ptr);
    void *d_dY = reinterpret_cast<void *>(dY_ptr);
    void *d_dW = reinterpret_cast<void *>(dW_ptr);

    int8_t *d_workspace = nullptr;
    cudaMalloc((void**)&d_workspace, workspace_size);
    // 5. Perform convolution (depthwise)
    float alpha = 1.0f, beta = 0.0f;
    CHECK_CUDNN_ERR(cudnnConvolutionBackwardFilter(handle, &alpha, input_desc, d_X, grad_desc, d_dY, conv_desc, algo, d_workspace, workspace_size, &beta, filter_desc, d_dW));
...
void conv2d_bpw(MyTensor Activation, MyTensor dY, MyTensor dWeight, uint padding, uint stride, uint dilation, uint groups) {
    ...
    auto Weight_shape = dWeight.get_dims();
    ...
    bpw_be_devicec(handle, {n,c,h,w,k,r,s,stride,dilation,padding,groups}, Activation.data_ptr(), dY.data_ptr(), dWeight.data_ptr(), be_type_switch(Activation.data_type()), be_type_switch(dY.data_type()));
